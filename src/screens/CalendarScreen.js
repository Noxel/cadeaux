...
class CalendarScreen extends Component {
    ...
    doIHaveEventOnThisDate(day) {
        ...
        this.props.dates.map(value => {
            ...
            return dateofmap.getUTCDate() === day.date.getUTCDate() + 1 &&
                dateofmap.getUTCMonth() === day.date.getUTCMonth() &&
                dateofmap.getUTCFullYear() === day.date.getUTCFullYear()
                ? (res = true)
                : null;
        });
        return res;
    ...
    getIDOfDay(day) {
        ...
        this.props.dates.map(value => {
            const dateofmap = new Date(value.date);
            return dateofmap.getUTCDate() === day.getUTCDate() &&
                dateofmap.getUTCMonth() === day.getUTCMonth() &&
                dateofmap.getUTCFullYear() === day.getUTCFullYear()
                ? (res = value.id)
                : null;
        });
        return res;
    ...
    requestDateEvent(day) {
        ...
        this.props.dates.map(value => {
            const dateofmap = new Date(value.date);
            return dateofmap.getUTCDate() === day.getUTCDate() + 1 &&
                dateofmap.getUTCMonth() === day.getUTCMonth() &&
                dateofmap.getUTCFullYear() === day.getUTCFullYear()
                ? (event = value)
                : null;
        ...
        this.setState({
            openScrollTabs: event.id !== undefined,
            idDate: event.id,
        ...
        return (
            <>
                <Calendar
                    ...
                    onClickDay={value => {
                        this.requestDateEvent(value);
                    }}
                    tileClassName={date => (this.doIHaveEventOnThisDate(date) ? classes.eventTiles : classes.tiles)}
                />
                {this.props.openAddDateDialog ? <AddDateDialog /> : null}
                {this.props.openInfoDateDialog ? (
                    <InfoDateDialog
                ...
                {this.props.openDelDateDialog ? (
                    <DelDateDialog date={this.getIDOfDay(this.state.dateEventClicked)} />
                ) : null}
                {this.state.openScrollTabs ? (
                    <ScrollTabs
                        id={this.state.idDate}
                        date={this.state.dateEventClicked}
                        budget={this.state.budget}
                ...
                {this.props.openAddContactDialog ? (
                    <AddContactDialog
                ...
                {this.props.openInfoEventDialog ? <InfoEventDialog idDate={this.state.idDate} /> : null}
